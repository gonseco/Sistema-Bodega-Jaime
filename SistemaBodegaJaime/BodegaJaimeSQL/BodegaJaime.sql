...
BEGIN
    ...
    WHERE p.estado = 1
      ...
      AND (p_marca     IS NULL OR m.nombre  LIKE CONCAT('%', p_marca,     '%'))
      AND (p_categoria IS NULL OR c.nombre  LIKE CONCAT('%', p_categoria, '%'))
      AND (p_unidad    IS NULL OR um.abreviacion LIKE CONCAT('%', p_unidad,    '%'));
END$$
DELIMITER ; -- funciona
...
BEGIN
    ...
    ELSE
        ...
        UPDATE venta SET total = total + v_subtotal WHERE id_venta = p_id_venta;
 
        SET p_resultado = 'OK: Detalle registrado';
    END IF;
END$$
DELIMITER ; -- funciona
...
BEGIN
    ...
    ELSE
        ...
        SET p_resultado = 'OK: Stock reducido';
    END IF;
END$$
DELIMITER ; -- funciona
...
CREATE PROCEDURE sp_producto_crear(
    ...
    IN  p_nombre              VARCHAR(100),
    IN  p_stock_actual        DECIMAL(10,3),
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 FROM producto WHERE LOWER(nombre) = LOWER(TRIM(p_nombre))
    ) THEN
    ...
    ELSEIF p_stock_minimo IS NULL OR p_stock_minimo < 0 THEN
        SET p_resultado = 'ERROR: El stock mínimo debe ser mayor o igual que 0.';
    ELSEIF p_precio_venta IS NULL OR p_precio_venta <= 0 THEN
        SET p_resultado = 'ERROR: El precio de venta es incorrecto.';
    ELSE
        SET v_codigo_barras = fn_generar_codigo_barras();
        INSERT INTO producto
            (id_marca, id_categoria, id_unidad_de_medida, nombre,
             stock_actual, stock_minimo, precio_venta, codigo_barras)
        VALUES
            (p_id_marca, p_id_categoria, p_id_unidad_de_medida, TRIM(p_nombre),
             p_stock_actual, IFNULL(p_stock_minimo, 0), p_precio_venta, v_codigo_barras);
...
BEGIN
    ...
    ELSE
        ...
        UPDATE orden_compra SET total = total + v_subtotal WHERE id_orden_compra = p_id_orden_compra;
...
BEGIN
    ...
    DECLARE v_cantidad_pedida DECIMAL(10,3);
 
    IF p_cantidad IS NULL OR p_cantidad <= 0 THEN
        SET p_resultado = 'ERROR: La cantidad debe ser mayor a 0.';
    ELSE
        SELECT id_orden_compra INTO v_id_orden FROM guia_remision WHERE id_guia_remision = p_id_guia_remision;
 
        SELECT cantidad INTO v_cantidad_pedida FROM detalle_orden_compra
        ...
        ELSE
            ...
            UPDATE detalle_orden_compra SET cantidad_recibida = cantidad
            WHERE id_orden_compra = v_id_orden AND id_producto = p_id_producto;
...
BEGIN
    ...
    ELSEIF p_precio_unitario IS NULL OR p_precio_unitario <= 0 THEN
        SET p_resultado = 'ERROR: El precio unitario debe ser mayor a 0.';
    ELSE
        SELECT stock_actual INTO v_stock FROM producto WHERE id_producto = p_id_producto AND estado = 1;
        ...
        ELSE
            ...
            UPDATE producto SET stock_actual = stock_actual - p_cantidad WHERE id_producto = p_id_producto;
            UPDATE comprobante SET total = total + v_subtotal WHERE id_comprobante = p_id_comprobante;
